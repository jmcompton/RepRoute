...
router.get('/data', async (req, res) => {
  ...
  try {
    ...
    const hot = await pool.query("SELECT COUNT(*) FROM prospects WHERE user_id=$1 AND priority='High'", [uid]);
    ...
    const hotProspects = await pool.query(
      "SELECT * FROM prospects WHERE user_id=$1 AND priority IN ('High','Medium') ORDER BY CASE priority WHEN 'High' THEN 1 ELSE 2 END, pipeline_stage DESC LIMIT 6", [uid]);
