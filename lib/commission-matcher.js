...
function splitLocation(raw) {
  ...
  // Trailing " - City" (space-hyphen-space) when no comma-city was found. Same
  // guards as the comma branch. The surrounding-space requirement means embedded
  // hyphens (MCS-Decatur, Cell-Pak, 31-W, Alum-A-Pole, T-Gale) are never split.
  if (!city) {
    const hm = s.match(/\s-\s+([^-]+?)\s*$/);
    if (hm) {
      const seg = hm[1].trim();
      const segCore = seg.toLowerCase().replace(LEGAL_SUFFIX_RX, '').replace(/[^a-z0-9 ]/g, '').trim();
      const before = s.slice(0, hm.index).trim();
      if (before && segCore && !/^(inc|llc|co|corp|ltd|company|incorporated)$/.test(seg.toLowerCase().replace(/[^a-z]/g, ''))) {
        city = seg;
        s = before;
      }
...
// ── City helpers (shared by matcher + store) ─────────────────────────────────
// entityCity: a record's city — the explicit city field if present, else the
// city embedded in its name (e.g. "Service Partners - Birmingham").
function entityCity(name, cityField) {
  if (cityField != null && String(cityField).trim()) return String(cityField).trim();
  return splitLocation(name).city || null;
}
// cityCompatible: unknown city on either side → compatible; else case-insensitive equal.
...
function matchClusterToAccount(cluster, accounts, mapByNorm) {
  const clusterCity = (cluster.cities && cluster.cities[0]) || null;

  // (a) learned map — any normalized key already linked? auto-match. BUT the map
  // key is city-blind (normalizeCustomer strips the city), so two different-city
  // branches collapse to one key. Guard it: if the mapped account is a known but
  // city-INCOMPATIBLE account, skip the map and fall through to the (city-guarded)
  // fuzzy step so a Huntsville cluster finds its own Huntsville account.
  for (const key of cluster.normalized_keys) {
    if (mapByNorm && mapByNorm.has(key)) {
      ...
      if (acct && !cityCompatible(clusterCity, entityCity(acct.company, acct.city))) continue;
      return {
  ...
  // but never across cities (a Huntsville cluster must not link to a Birmingham branch).
  let best = null, bestScore = 0;
  for (const a of accounts) {
    const accountCity = entityCity(a.company, a.city);
    if (!cityCompatible(clusterCity, accountCity)) continue;
