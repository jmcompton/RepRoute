...
const { clusterCommissionLines, matchClusterToAccount, normalizeCustomer, pairScore, entityCity, cityCompatible, splitLocation } = require('./commission-matcher');
...
async function findOrCreateAccount(client, repUserId, name, city, state, claimed) {
  ...
  // Store a CLEAN company name. Commission cells often bake the location into the
  // customer string ("Heritage Insulation, Auburn, AL"); strip that trailing
  // ", City, ST" so company is just the business name, and keep city/state in
  // their own columns (preferring the passed values, falling back to the parsed
  // ones). This keeps later Places enrichment queries un-polluted.
  const split = splitLocation(name);
  const company = (split.core || String(name || '')).trim();
  const finalCity = (city && String(city).trim()) || split.city || null;
  const finalState = (state && String(state).trim()) || split.state || null;
  const company_type = resolveCompanyType('');
  // NOTE: do NOT stamp last_activity_at on import. A commission import is
  // HISTORICAL data, not a fresh touch — stamping NOW() makes Reconnect think
  // every imported account was just contacted, so nobody ever goes "quiet".
  // Reconnect derives real recency from actual signals (commission period,
  // calls, visits, samples, quotes) instead. Leaving last_activity_at NULL here.
  // contact_status='needs_info': a freshly created import account has no phone or
  // address (we never fabricate them), so it joins the "Find missing info" queue.
  // High-confidence matches reuse an EXISTING account above and never reach here,
  // so their phone/address/contact_status are left untouched.
  const ins = await client.query(
    `INSERT INTO prospects
       (user_id, company, category, company_type, city, state, source, data_status, contact_status)
     VALUES ($1,$2,$3,$4,$5,$6,'Commission Import','Verified CRM Data','needs_info')
     RETURNING id`,
    [repUserId, company, 'Commission Import', company_type, finalCity, finalState]
  );
