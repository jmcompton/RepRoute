...
async function findOrCreateAccount(client, repUserId, name, city, state) {
  ...
  try {
    ...
    for (const d of (decisions || [])) {
      ...
      if (d.action === 'existing') {
        ...
        accountId = await findOrCreateAccount(client, repUserId, d.new_name, d.new_city, d.new_state);
      } else {
